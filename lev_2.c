...
int Calc_Levenshtein_Distance(char *first_arg, char *second_arg){
    ...
    for(int j = 0; j < first_arg_length + 1; j++){
        array_lev[j][0] = j;
    }
    for(int k = 0; k < second_arg_length + 1; k++){
        array_lev[0][k] = k;
    }

    //main algorithm
    for (int l = 1; l < first_arg_length + 1; l++) {
    ...
    if (array_lev[first_arg_length][second_arg_length] != 0){
        return array_lev[first_arg_length][second_arg_length];
    }
    else{
        printf("fails");
        return 0;
